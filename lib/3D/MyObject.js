...
export default class MyObject {
    ...
    fragmentShaderSource = `
        ...
        void main() {
            ...
            if (uLightStatus == 1) {
                ...
                // Apply the shading
                gl_FragColor = vec4(ambient * vColor, 1.);
            }
        }
    `;
    constructor(gl) {
    ...
    draw(gl, light, viewMatrix) {
        ...
        // Update light position which has been rotated using quarternion


        gl.uniform3fv(uLightPosition, newLightPosition);    // light position

        // Light status
        var uLightStatus = gl.getUniformLocation(this.shaderProgram, "uLightStatus");
        if (light.status == 'on') {
            gl.uniform1i(uLightStatus, 1);
        }
        else {
            gl.uniform1i(uLightStatus, 0);
        }
